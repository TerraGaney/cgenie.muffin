...
if (ctrl_debug_init > 0) then
  ...
  ! ****************************************************************************************************************************** !
  ! LOAD CARBONATE CONSTANT LOOK-UP TABLES - MCS K1
  SUBROUTINE sub_load_gem_MCS_lookup_K1()
    USE gem_util, ONLY: sub_report_error
    !USE genie_util, ONLY: check_unit, check_iostat
    ! local variables
    INTEGER::a,b,d,e
    ...
    ALLOCATE(lookup_gem_MCS_k1( &
     ...
     IF (error /= 0) THEN
        CALL sub_report_error( &
             & 'initialize_gem','initialise_gem', &
             & 'Could not allocate space for MCS K1 look-up table array', &
             & 'STOPPING', &
             & (/const_real_zero/),.TRUE. &
             & )
     ENDIF
    ! *** read in look-up data ***
    loc_filename = '../../cgenie.muffin/genie-main/data/input/MCS_lookup_K1.dat'
    !call check_unit(in,__LINE__,__FILE__)
    OPEN(unit=in,file=loc_filename,action='read',iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
    ! read in data
    DO a = lookup_i_Ca_min,lookup_i_Ca_max,1
       DO b = lookup_i_Mg_min,lookup_i_Mg_max,1
          DO d = lookup_i_sal_min,lookup_i_sal_max,1
             DO e = lookup_i_temp_min,lookup_i_temp_max,1
                READ(unit=in,FMT='(D23.16)',iostat=ios) lookup_gem_MCS_k1(a,b,d,e)
                !print*, a,b,d,e,lookup_gem_MCS_k1(a,b,d,e)
                !call check_iostat(ios,__LINE__,__FILE__)
             END DO
          END DO
       END DO
    ...
    CLOSE(unit=in,iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
  END SUBROUTINE sub_load_gem_MCS_lookup_K1
  ! ****************************************************************************************************************************** !

  ! ****************************************************************************************************************************** !
  ! LOAD CARBONATE CONSTANT LOOK-UP TABLES - MCS K2
  SUBROUTINE sub_load_gem_MCS_lookup_K2()
    USE gem_util, ONLY: sub_report_error
    !USE genie_util, ONLY: check_unit, check_iostat
    ! local variables
    INTEGER::a,b,d,e
    ...
    ALLOCATE(lookup_gem_MCS_k2( &
     ...
     ENDIF
    ! *** read in look-up data ***
    loc_filename = '../../cgenie.muffin/genie-main/data/input/MCS_lookup_K2.dat'
    !call check_unit(in,__LINE__,__FILE__)
    OPEN(unit=in,file=loc_filename,action='read',iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
    ! read in data
    DO a = lookup_i_Ca_min,lookup_i_Ca_max,1
       DO b = lookup_i_Mg_min,lookup_i_Mg_max,1
          DO d = lookup_i_sal_min,lookup_i_sal_max,1
             DO e = lookup_i_temp_min,lookup_i_temp_max,1
                READ(unit=in,FMT='(D23.16)',iostat=ios) lookup_gem_MCS_k2(a,b,d,e)
                !call check_iostat(ios,__LINE__,__FILE__)
             END DO
          END DO
       END DO
    ...
    CLOSE(unit=in,iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
  END SUBROUTINE sub_load_gem_MCS_lookup_K2
  ! ****************************************************************************************************************************** !

  ! ****************************************************************************************************************************** !
  ! LOAD CARBONATE CONSTANT LOOK-UP TABLES - MCS Kcal
  SUBROUTINE sub_load_gem_MCS_lookup_Kcal()
    USE gem_util, ONLY: sub_report_error
    !USE genie_util, ONLY: check_unit, check_iostat
    ! local variables
    INTEGER::a,b,d,e
    ...
    ALLOCATE(lookup_gem_MCS_kcal( &
     ...
     IF (error /= 0) THEN
        CALL sub_report_error( &
             & 'initialize_gem','initialise_gem', &
             & 'Could not allocate space for MCS Kcal look-up table array', &
             & 'STOPPING', &
             & (/const_real_zero/),.TRUE. &
             & )
     ENDIF
    ! *** read in look-up data ***
    loc_filename = '../../cgenie.muffin/genie-main/data/input/MCS_lookup_KspC.dat'
    !call check_unit(in,__LINE__,__FILE__)
    OPEN(unit=in,file=loc_filename,action='read',iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
    ! read in data
    DO a = lookup_i_Ca_min,lookup_i_Ca_max,1
       DO b = lookup_i_Mg_min,lookup_i_Mg_max,1
          DO d = lookup_i_sal_min,lookup_i_sal_max,1
             DO e = lookup_i_temp_min,lookup_i_temp_max,1
                READ(unit=in,FMT='(D23.16)',iostat=ios) lookup_gem_MCS_kcal(a,b,d,e)
                !call check_iostat(ios,__LINE__,__FILE__)
             END DO
          END DO
       END DO
    ...
    CLOSE(unit=in,iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
  END SUBROUTINE sub_load_gem_MCS_lookup_Kcal
  ! ****************************************************************************************************************************** !

  ! ****************************************************************************************************************************** !
  ! LOAD CARBONATE CONSTANT LOOK-UP TABLES - MCS Karg
  SUBROUTINE sub_load_gem_MCS_lookup_Karg()
    USE gem_util, ONLY: sub_report_error
    !USE genie_util, ONLY: check_unit, check_iostat
    ! local variables
    INTEGER::a,b,d,e
    ...
    ALLOCATE(lookup_gem_MCS_karg( &
     ...
     IF (error /= 0) THEN
        CALL sub_report_error( &
             & 'initialize_gem','initialise_gem', &
             & 'Could not allocate space for MCS Karg look-up table array', &
             & 'STOPPING', &
             & (/const_real_zero/),.TRUE. &
             & )
     ENDIF
    ! *** read in look-up data ***
    loc_filename = '../../cgenie.muffin/genie-main/data/input/MCS_lookup_KspA.dat'
    !call check_unit(in,__LINE__,__FILE__)
    OPEN(unit=in,file=loc_filename,action='read',iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
    ! read in data
    DO a = lookup_i_Ca_min,lookup_i_Ca_max,1
       DO b = lookup_i_Mg_min,lookup_i_Mg_max,1
          DO d = lookup_i_sal_min,lookup_i_sal_max,1
             DO e = lookup_i_temp_min,lookup_i_temp_max,1
                READ(unit=in,FMT='(D23.16)',iostat=ios) lookup_gem_MCS_karg(a,b,d,e)
                !call check_iostat(ios,__LINE__,__FILE__)
             END DO
          END DO
       END DO
    ...
    CLOSE(unit=in,iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
  END SUBROUTINE sub_load_gem_MCS_lookup_Karg
  ! ****************************************************************************************************************************** !

  ! ****************************************************************************************************************************** !
  ! LOAD CARBONATE CONSTANT LOOK-UP TABLES - MCS Kw
  SUBROUTINE sub_load_gem_MCS_lookup_Kw()
    USE gem_util, ONLY: sub_report_error
    !USE genie_util, ONLY: check_unit, check_iostat
    ! local variables
    INTEGER::a,b,d,e
    ...
    ALLOCATE(lookup_gem_MCS_kW( &
     ...
     IF (error /= 0) THEN
        CALL sub_report_error( &
             & 'initialize_gem','initialise_gem', &
             & 'Could not allocate space for MCS Kw look-up table array', &
             & 'STOPPING', &
             & (/const_real_zero/),.TRUE. &
             & )
     ENDIF
    ! *** read in look-up data ***
    loc_filename = '../../cgenie.muffin/genie-main/data/input/MCS_lookup_Kw.dat'
    !call check_unit(in,__LINE__,__FILE__)
    OPEN(unit=in,file=loc_filename,action='read',iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
    ! read in data
    DO a = lookup_i_Ca_min,lookup_i_Ca_max,1
       DO b = lookup_i_Mg_min,lookup_i_Mg_max,1
          DO d = lookup_i_sal_min,lookup_i_sal_max,1
             DO e = lookup_i_temp_min,lookup_i_temp_max,1
                READ(unit=in,FMT='(D23.16)',iostat=ios) lookup_gem_MCS_kW(a,b,d,e)
                !call check_iostat(ios,__LINE__,__FILE__)
             END DO
          END DO
       END DO
    ...
    CLOSE(unit=in,iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
  END SUBROUTINE sub_load_gem_MCS_lookup_Kw
  ! ****************************************************************************************************************************** !

  ! ****************************************************************************************************************************** !
  ! LOAD CARBONATE CONSTANT LOOK-UP TABLES - MCS K
  SUBROUTINE sub_load_gem_MCS_lookup_K0()
    USE gem_util, ONLY: sub_report_error
    !USE genie_util, ONLY: check_unit, check_iostat
    ! local variables
    INTEGER::a,b,d,e
    ...
    ALLOCATE(lookup_gem_MCS_k( &
     ...
     IF (error /= 0) THEN
        CALL sub_report_error( &
             & 'initialize_gem','initialise_gem', &
             & 'Could not allocate space for MCS K0 look-up table array', &
             & 'STOPPING', &
             & (/const_real_zero/),.TRUE. &
             & )
     ENDIF
    ! *** read in look-up data ***
    loc_filename = '../../cgenie.muffin/genie-main/data/input/MCS_lookup_K0.dat'
    !call check_unit(in,__LINE__,__FILE__)
    OPEN(unit=in,file=loc_filename,action='read',iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
    ! read in data
    DO a = lookup_i_Ca_min,lookup_i_Ca_max,1
       DO b = lookup_i_Mg_min,lookup_i_Mg_max,1
          DO d = lookup_i_sal_min,lookup_i_sal_max,1
             DO e = lookup_i_temp_min,lookup_i_temp_max,1
                READ(unit=in,FMT='(D23.16)',iostat=ios) lookup_gem_MCS_k(a,b,d,e)
                !call check_iostat(ios,__LINE__,__FILE__)
             END DO
          END DO
       END DO
    ...
    CLOSE(unit=in,iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
  END SUBROUTINE sub_load_gem_MCS_lookup_K0
  ! ****************************************************************************************************************************** !

  ! ****************************************************************************************************************************** !
  ! LOAD CARBONATE CONSTANT LOOK-UP TABLES - MCS Kb
  SUBROUTINE sub_load_gem_MCS_lookup_Kb()
    USE gem_util, ONLY: sub_report_error
    !USE genie_util, ONLY: check_unit, check_iostat
    ! local variables
    INTEGER::a,b,d,e
    ...
    ALLOCATE(lookup_gem_MCS_kb( &
     ...
     ENDIF
    ! *** read in look-up data ***
    loc_filename = '../../cgenie.muffin/genie-main/data/input/MCS_lookup_Kb.dat'
    !call check_unit(in,__LINE__,__FILE__)
    OPEN(unit=in,file=loc_filename,action='read',iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
    ! read in data
    DO a = lookup_i_Ca_min,lookup_i_Ca_max,1
       DO b = lookup_i_Mg_min,lookup_i_Mg_max,1
          DO d = lookup_i_sal_min,lookup_i_sal_max,1
             DO e = lookup_i_temp_min,lookup_i_temp_max,1
                READ(unit=in,FMT='(D23.16)',iostat=ios) lookup_gem_MCS_kb(a,b,d,e)
                !call check_iostat(ios,__LINE__,__FILE__)
             END DO
          END DO
       END DO
    ...
    CLOSE(unit=in,iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
  END SUBROUTINE sub_load_gem_MCS_lookup_Kb
  ! ****************************************************************************************************************************** !

  ! ****************************************************************************************************************************** !
  ! LOAD CARBONATE CONSTANT LOOK-UP TABLES - MCS KSO4
  SUBROUTINE sub_load_gem_MCS_lookup_KSO4()
    USE gem_util, ONLY: sub_report_error
    !USE genie_util, ONLY: check_unit, check_iostat
    ! local variables
    INTEGER::a,b,d,e
    ...
    ALLOCATE(lookup_gem_MCS_kSO4( &
     ...
     ENDIF
    ! *** read in look-up data ***
    loc_filename = '../../cgenie.muffin/genie-main/data/input/MCS_lookup_KSO4.dat'
    !call check_unit(in,__LINE__,__FILE__)
    OPEN(unit=in,file=loc_filename,action='read',iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
    ! read in data
    DO a = lookup_i_Ca_min,lookup_i_Ca_max,1
       DO b = lookup_i_Mg_min,lookup_i_Mg_max,1
          DO d = lookup_i_sal_min,lookup_i_sal_max,1
             DO e = lookup_i_temp_min,lookup_i_temp_max,1
                READ(unit=in,FMT='(D23.16)',iostat=ios) lookup_gem_MCS_kSO4(a,b,d,e)
                !call check_iostat(ios,__LINE__,__FILE__)
             END DO
          END DO
       END DO
    ...
    CLOSE(unit=in,iostat=ios)
    !call check_iostat(ios,__LINE__,__FILE__)
  END SUBROUTINE sub_load_gem_MCS_lookup_KSO4
  ! ****************************************************************************************************************************** !
